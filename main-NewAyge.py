...
class FileOrganizer:
    ...
    def __init__(self):
        ...
        self.image_inference: Optional[NexaVLMInference] = None
        self.text_inference: Optional[NexaTextInference] = None
...
def get_mode_selection():
    ...
    while True:
        ...
        print("2. By Date") 
        print("3. By Type")
        print("4. Checklist Mode (One Step at a Time)")
...
def manage_checklist(operations):
    """Handle checklist mode, presenting one operation at a time."""
    completed = 0
    total = len(operations)
    
    for i, operation in enumerate(operations, 1):
        print(f"\nChecklist Item {i}/{total}:")
        print(f"Source: {operation['source']}")
        print(f"Destination: {operation['destination']}")
        print(f"Operation: {operation['link_type']}")
        
        while True:
            action = input("Enter 'c' to complete, 's' to skip, or '/exit' to quit: ").lower()
            if action == 'c':
                # Execute the operation
                os.makedirs(os.path.dirname(operation['destination']), exist_ok=True)
                if operation['link_type'] == 'hardlink':
                ...
                print(f"Checklist progress: {completed}/{total} items completed.")
                return
            else:
                print("Invalid input. Please try again.")
    
    print(f"\nChecklist complete! {completed}/{total} items processed.")
    ...
    while True:
        ...
        while True:
            ...
            if mode == 'content':
                ...
                # Process files by type first to generate operations
                operations = process_files_by_type(file_paths, output_path, dry_run=False, silent=silent_mode, log_file=log_file)
                manage_checklist(operations)
                continue
