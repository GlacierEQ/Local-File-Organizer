...
class ScanWindow:
    ...
    def setup_ui(self):
        ...
        path_frame = ttk.Frame(self.progress_frame)
        ...
        self.progress = ttk.Progressbar(
            self.progress_frame, orient=tk.HORIZONTAL, length=600, mode="determinate"
        )
        self.progress.pack(pady=10)

        # Status label
        self.status_label = ttk.Label(
            self.progress_frame, text="Ready to scan", font=("Arial", 10)
        )
        ...
        self.start_button = ttk.Button(
            self.control_frame, text="Start Scan", command=self.start_scan
        )
    ...
    def start_scan(self):
        ...
        if not path or not os.path.exists(path):
            self.status_label.config(text="Please select a valid path")
